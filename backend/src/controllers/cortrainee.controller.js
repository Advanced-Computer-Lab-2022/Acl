...
//clientID     459689521337-pkpimajok78e5f00jhlubvtr28smvm0p.apps.googleusercontent.com
  ...
  const answerMcq = async (req, res) => {
      ...
      const report=async(req,res)=>{
        ...
        try{
          reports.create({name: reportname, type: type, course:c,corporatetrainee:id,Description:description})
      ...
      const followups=async(req,res)=>{
       ...
       const {title}=req.body
        try{
          const r=await reports.findOne({$and: [{name:title},{corporatetrainee:id}]})
          if(r.status=="pending"||r.status=="unseen"){
            const{followup,number}=req.body
            await reports.updateOne({$and: [{name:title},{corporatetrainee:id}]}, {
              $push: {
                followups: {
                  number:number,
...
module.exports={ChangePass,filterAllCoursesBySubject,finddCourses,findmyCourses,viewall,selectcountry,reset,resetPost,forgetPassPost,forgetPass,answerMcq,showAnswers, report,
  viewreports,
  followups,requestaccess};